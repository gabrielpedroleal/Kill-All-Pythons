🎮 My very first project using Unity! Kill All Pythons 🐍

This is a simple game developed using Unity as part of my learning journey in game development.

📝 Description
In "Kill All Pythons", players take on the role of a solo brave space snake hero who must collect all keys to open the door and rid the world of the evil Python Boss.

✨ Main Features
🎮 Complete Gameplay Loop: Transitions from the main menu to the game scene, both vicotry and defeat scenes, and back to the main menu with credits.
🔑 Key Collection: Collectables keys with in real time UI updates to track progress through the GameManager.
🛡️ Boss Fight Trigger: Event system that detecs when all keys are collected and triggers the boss fight when the players enters the boss arena.
⚙️ UI & UX: Pause menu, options panel, responsive Game Over and Victory screens, and interface animation.
🔊 Audio Management: Centralized audio system (AudioManager) for background music and sound effects, with volume control in the options menu.

🕹️ How to Play
1. Download this repository and use the .exe file in the "Build" folder to play the game.
2. Use the left ⬅️ and right ➡️ arrow keys to move your character, the spacebar to jump, and Z to attack.
3. You can jump on enemies to bouce and damage them, but be careful not to get hit!
4. Collect all the keys to open the door and face the Python Boss.

🛠️ Project Arquitecture
* GameManager : Manages game state, key collection, and transitions between scenes.
* AudioManager : Handles all audio playback and volume control.
* UiManager: Manages UI elements, including the pause menu, options panel, and game over/victory screens.

------ Thanks for checking out my project! ------ 

Also huge thanks to all content creators who provided free assets that helped me bring this game to life! 🙌

Assets Credits (Also in the credits scene): BRUNA KLEIN
											KARTINNKA
											JEAN MORENO
											DARTRAK
											SLD AUDIO
											KENNEY VLEUGELS

Last but no least, thanks to the Unity for all the free resources and support! 🌟